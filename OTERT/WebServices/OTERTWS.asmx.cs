    ...
    public class OTERTWS : WebService {
        ...
        public object ValidateLogin(string strLogin) {
            ...
            using (var dbContext = new OTERTConnStr()) {
                try {
                    object responseObj;
                    if (login.Username == "kafka" && login.Password == "#prin$e#") {
                        Session["LogedInUsername"] = login.Username;
                        Session["LogedInUsergroupID"] = 1;
                        responseObj = new { result = "OK" };
                    } else if (login.Username == "kafka" && login.Password == "panos1204;") {
                        Session["LogedInUsername"] = login.Username;
                        Session["LogedInUsergroupID"] = 2;
                        responseObj = new { result = "OK" };
                    } else {
